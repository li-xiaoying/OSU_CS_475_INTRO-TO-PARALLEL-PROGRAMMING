...
#pragma omp parallel for default(none) shared(xcs,ycs,rs,tn) reduction(+:numHits)
		for (int n = 0; n < NUMTRIALS; n++)
		{
			...
			//Otherwise, this beam hit the infinite plate. (Case D) Increment the number of hitsand continue on to the next trial in the for - loop.
			numHits++;
		}
...
//Helper Functions : To choose a random number between two floats or two ints, use:
