...
int main(int argc, char* argv[])
{
	...
	float* hD = new float[NUM_ELEMENTS];	// add for part 2
	...
	cl_mem dC = clCreateBuffer(context, CL_MEM_READ_ONLY, dataSize, NULL, &status);	// change CL_MEM_WRITE_ONLY to CL_MEM_READ_ONLY for part 2
	if (status != CL_SUCCESS)
	...
	// change globalWorkSize[3] to globalWorkSize[4] for part 2
	...
	status = clEnqueueReadBuffer(cmdQueue, dD, CL_TRUE, 0, dataSize, hD, 0, NULL, NULL);	// change dC, hC to dD, hD for part 2
	if (status != CL_SUCCESS)
	...
	clReleaseMemObject(dD);	// add for part 2

	delete[] hA;
	delete[] hB;
	delete[] hC;
	delete[] hD;	// add for part 2
