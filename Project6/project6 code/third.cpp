...
int main(int argc, char* argv[])
{
	...
	size_t cSize = NUM_WORK_GROUPS * sizeof(float);	// add for part 3
	...
	cl_mem dC = clCreateBuffer(context, CL_MEM_WRITE_ONLY, cSize, NULL, &status);	// change dataSize to cSize for part 3
	if (status != CL_SUCCESS)
	...
	status = clSetKernelArg(kernel, 2, LOCAL_SIZE * sizeof(float), NULL);	// local ¡°prods¡± array is dimensioned the size of each work-group
	if (status != CL_SUCCESS)
