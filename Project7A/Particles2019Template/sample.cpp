...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Xiaoying Li" };
...
float MaxMegaParticles = 0.;	/* get ready to record the maximum performance */
...
Display( )
{
	...
	if( ShowPerformance )
	{
		...
		float megaPartivles = NUM_PARTICLES / ElapsedTime / 1000000.;
		sprintf( str, "%6.1f MegaParticles/Sec", megaPartivles);
		if (megaPartivles < INFINITY && megaPartivles > MaxMegaParticles) {
			MaxMegaParticles = megaPartivles;
		}
...
InitLists( )
{
	...
	// do this for yourself...
		...
		glPushMatrix();
		glTranslatef(0.f, -3500.f, 0.f);
		glutWireSphere(2000.f, 100, 100);
		glPopMatrix();
...
Quit( )
{
	printf("Peak Performance = %f MegaParticles/Sec\n", MaxMegaParticles);	/* print out the peak performance */
